...
与此同时，系统已经完成了一轮面向运行链路的本地 bootstrap 训练。该训练在本机 V100 主机上执行，当前实际使用的是从树莓派历史目录回收的 160 张共享合成单字样本，用于导出 \texttt{quality\_scorer\_regular.onnx} 与 \texttt{quality\_scorer\_running.onnx} 两套模型产物。需要强调的是，这一轮训练的目标是优先验证“双模型文件真实存在、设备端可装载、树莓派可完成完整评测与测试”这一工程闭环，而不是宣称行书模型已经完成正式数据支撑。当前 running 模型仍属于共享合成样本驱动的 bootstrap 版本，后续仍需补充真实行书样本、人工评分对照和专家复评后再进入正式重训阶段。
