...
KIOSK_MODE="${INKPI_KIOSK_MODE:-stack}"

if [ "${KIOSK_MODE}" = "stack" ]; then
    SESSION_SCRIPT="${PROJECT_DIR}/scripts/inkpi-stack-kiosk-session.sh"
else
    SESSION_SCRIPT="${PROJECT_DIR}/scripts/inkpi-kiosk-session.sh"
fi
...
if [ -z "\${SSH_TTY:-}" ] && [ -z "\${DISPLAY:-}" ] && [ "\$(tty)" = "/dev/tty1" ]; then
    exec startx "${SESSION_SCRIPT}" -- :0 vt1 -keeptty
fi
${MARK_END}
EOF
...
chmod +x "${PROJECT_DIR}/scripts/inkpi-launch.sh" "${PROJECT_DIR}/scripts/inkpi-kiosk-session.sh" "${PROJECT_DIR}/scripts/inkpi-stack-kiosk-session.sh"
